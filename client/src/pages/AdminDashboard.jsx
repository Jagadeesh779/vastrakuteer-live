...
const AdminDashboard = () => {
    ...
    const handleExportOrders = () => {
        const headers = ['Order ID', 'Customer', 'Email', 'Items', 'Total', 'Coupon', 'Discount', 'Shipping', 'Status', 'Date'];
    ...
    const handlePrintLabel = (order) => {
        ...
        win.document.write(`
            <html><head><title>Shipping Label - #${order._id.slice(-6).toUpperCase()}</title>
            ...
            <style>
                @media print {
                    ...
                    body { margin: 0; padding: 0.1in; -webkit-print-color-adjust: exact; print-color-adjust: exact; width: 4in; height: 6in; }
                }
                body { font-family: 'Inter', sans-serif; background: #fff; color: #000; width: 4in; height: 6in; margin: 0 auto; box-sizing: border-box; overflow: hidden; }
                .label-container { border: 2px solid #000; padding: 0.25in; display: flex; flex-direction: column; height: 100%; box-sizing: border-box; position: relative; z-index: 1; overflow: hidden; }
                .header { font-size: 20px; font-weight: 800; border-bottom: 3px solid #000; padding-bottom: 5px; margin-bottom: 8px; text-transform: uppercase; text-align: center; letter-spacing: 2px; }
                .section { border-bottom: 1px solid #000; padding: 8px 0; }
                .section.no-border { border-bottom: none; }
                .label-title { font-size: 9px; text-transform: uppercase; color: #444; margin-bottom: 2px; font-weight: 700; letter-spacing: 0.5px;}
                .bold-text { font-size: 13px; font-weight: 700; margin-bottom: 1px; }
                .text { font-size: 11px; margin-bottom: 1px; color: #111; }
                .barcode-section { align-items: center; justify-content: center; display: flex; flex-direction: column; padding: 12px 0; border-bottom: 3px dashed #000; background: #fafafa;}
                .barcode { font-family: 'Libre Barcode 39 Text', cursive; font-size: 44px; transform: scaleY(1.3); margin: 5px 0; font-weight: normal; letter-spacing: 1px;}
                ...
                .footer { text-align: center; font-size: 10px; margin-top: auto; padding-top: 10px; font-weight: 600; border-top: 1px solid #000;}
            </style>
            </head><body onload="setTimeout(() => window.print(), 500)">
                <div class="label-container">
                    <!-- Transparent Watermark Background -->
                    <img src="${Logo.startsWith('http') ? Logo : window.location.origin + Logo}" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 160%; max-width: none; opacity: 0.15; z-index: 0; filter: grayscale(100%); pointer-events: none;" alt="" />
                    
                    <div class="header">VASTRA KUTEER</div>
                    
                    <div class="flex-row section">
                        <div class="flex-col">
                            <div class="label-title">Ship From:</div>
                            <div class="bold-text">Vastra Kuteer HQ</div>
                            <div class="text">123 Heritage Silk Road</div>
                            <div class="text">Textile Logistics Hub</div>
                            <div class="text">Surat, Gujarat 395002</div>
                            <div class="text">Ph: +91 99999 00000</div>
                        </div>
                        <div class="flex-col" style="border-left: 2px solid #000; padding-left: 8px;">
                            <div class="label-title">Ship To:</div>
                            <div class="bold-text" style="font-size: 15px;">${order.shippingAddress?.fullName || order.user?.fullName || 'Customer'}</div>
                            <div class="text">${order.shippingAddress?.street || ''}</div>
                            <div class="text">${order.shippingAddress?.city || ''} - ${order.shippingAddress?.zip || ''}</div>
                            <div class="text">${order.shippingAddress?.state || ''}, ${order.shippingAddress?.country || 'India'}</div>
                            <div class="text" style="margin-top: 4px; font-weight: bold;">Ph: ${order.shippingAddress?.phone || 'N/A'}</div>
                        </div>
                    </div>

                    <div class="barcode-section">
                        <div class="label-title">Tracking / Order No.</div>
                        <div class="barcode">*${(order.deliveryInfo?.trackingNumber || order._id.slice(-6)).toUpperCase()}*</div>
                        <div class="bold-text" style="letter-spacing: 4px; font-size: 16px;">${(order.deliveryInfo?.trackingNumber || order._id.slice(-6)).toUpperCase()}</div>
                    </div>

                    <div class="section">
                        <div class="label-title">Order Summary (Date: ${new Date(order.createdAt).toLocaleDateString('en-IN')})</div>
                        <table>
                            <thead><tr><th>Qty</th><th>Item</th><th>Size</th><th>Total</th></tr></thead>
                            <tbody>
                                ${order.items.map(i => `<tr><td><strong>${i.qty}</strong></td><td>${i.name}</td><td>${i.selectedSize || '-'}</td><td>₹${i.price * i.qty}</td></tr>`).join('')}
                            </tbody>
                        </table>
                    </div>

                    <div class="section no-border" style="display:flex; justify-content:space-between; align-items:flex-end;">
                        <div style="width: 45%;">
                            <div class="label-title">Payment Mode</div>
                            <div class="bold-text" style="font-size: 18px; border: 2px solid #000; display: inline-block; padding: 2px 8px; margin-top:2px;">PREPAID</div>
                        </div>
                        <div style="width: 50%; text-align: right;">
                            <div class="text">Subtotal: ₹${order.totalAmount + (order.discountAmount || 0) - (order.shippingFee || 0)}</div>
                            ${order.couponCode ? `<div class="text" style="font-weight:600;">Discount (${order.couponCode}): - ₹${order.discountAmount}</div>` : ''}
                            <div class="text">Shipping: ${(order.shippingFee || 0) > 0 ? '+ ₹' + order.shippingFee : 'Free'}</div>
                            <div class="bold-text" style="font-size: 18px; margin-top:4px; border-top: 1px solid #000; padding-top: 4px;">Grand Total: ₹${order.totalAmount}</div>
                        </div>
                    </div>
                    
                    <div class="footer">Please retain for returns/exchanges. Thank you for shopping with Vastra Kuteer!</div>
                </div>
            </body></html>
        `);
        win.document.close();
                                    ...
                                    {orders.map((order) => (
                                        <tr key={order._id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                ...
                                                {(order.couponCode || (order.shippingFee > 0)) && (
                                                    <div className="flex flex-col mt-1 gap-1 items-start">
                                                        {order.couponCode && <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-green-100 text-green-800">🎟️ {order.couponCode}</span>}
                                                        {(order.shippingFee || 0) > 0 && <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-blue-100 text-blue-800">🚚 +₹{order.shippingFee}</span>}
                                                    </div>
                                                )}
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
